...
const mockReturnUserAnddiscount = jest.fn(
    (name: UserType, discount: number) => {
        const res: IUserTypeDiscount = { userType: name, discount: discount }
        return res
    }
)

const mockReturnUserAndProductTypeDiscount = jest.fn(
    (name: UserType, productType: ProductType, discount: number) => {
        const res: IDiscountProductUser = {
            userType_id: name,
...
const mockReturnSumDiscount = jest.fn(
    (user: UserType, productType: ProductType) => {
        switch (user) {
            case 1:
                if (productType == ProductType.Toy) return 40
                else return 20
            case 2:
                if (productType == ProductType.Car) return 45
                else return 30
            case 0:
                return 10
            case 3:
...
describe('setup correct discount for user', () => {
    test('adds 20% for Premium', () => {
        ...
        const res20: IUserTypeDiscount = mockReturnUserAnddiscount(
            UserType.Premium,
            20
        )
    ...
    test('adds 10% for Standard', () => {
        const res10: IUserTypeDiscount = mockReturnUserAnddiscount(
            UserType.Standard,
            10
        )
    ...
    test('adds 30% for Gold', () => {
        const res30: IUserTypeDiscount = mockReturnUserAnddiscount(
            UserType.Gold,
            30
        )
    ...
    test('adds 50% for Free', () => {
        const res50: IUserTypeDiscount = mockReturnUserAnddiscount(
            UserType.Free,
            50
        )
...
describe('setup correct discount for user and product type', () => {
    test('adds 20% for Premium and Toy', () => {
        const res20: IDiscountProductUser =
            mockReturnUserAndProductTypeDiscount(
                UserType.Premium,
                ProductType.Toy,
                20
            )
    ...
    test('adds 15% for Gold and Car', () => {
        const res15: IDiscountProductUser =
            mockReturnUserAndProductTypeDiscount(
                UserType.Gold,
                ProductType.Car,
                15
...
describe('get correct sum discount', () => {
    test('get discount for Premium and Toy', () => {
        expect(mockReturnSumDiscount(UserType.Premium, ProductType.Toy)).toBe(
            40
        )
    })
    ...
    test('get discount for Standart and Food', () => {
        expect(mockReturnSumDiscount(UserType.Standard, ProductType.Food)).toBe(
            10
        )
    })
})
